...
interficie=$(ip -o link show | awk '{print $2,$9}' | grep UP | head -n 1 | awk '{print $1}' | sed 's/://')
...
sudo arp-scan -Nq --interface=$interficie --localnet |sed '1,2d' | sed '/^$/d' | sed '$d' | sed '$d' | sort -k2 | egrep "d8:cb|10:bf" | tee hosts20170228.csv | awk '{ print $1}' | tee ansible.hosts
